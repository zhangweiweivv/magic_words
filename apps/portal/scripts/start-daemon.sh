...
# ========== 清理函数 ==========
cleanup() {
    echo "[$(date)] [portal] Cleaning up child processes..."
    kill -- -$$ 2>/dev/null
    pkill -f "node.*portal/server" 2>/dev/null
    exit 0
}
...
# ========== 启动前清理残留 ==========
echo "[$(date)] [portal] Cleaning up stale processes..."
pkill -f "node.*portal/server" 2>/dev/null
sleep 1
...
if [ -n "$PID" ]; then
    echo "[$(date)] [portal] Port $PORT still occupied by PID $PID, killing..."
...
echo "[$(date)] [portal] Main process PID: $MAIN_PID"
wait $MAIN_PID
